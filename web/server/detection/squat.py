...
class SquatDetection:
    ...
    def detect(self, mp_results, image, timestamp) -> list:
        """
        Make Squat Errors detection
        """
        feedback = []
        try:
            # Debugging: Check if pose_landmarks attribute is present
            if not hasattr(mp_results, 'pose_landmarks') or mp_results.pose_landmarks is None:
                print("No pose landmarks detected")
                return ["No pose detected. Please adjust camera or lighting."]
        
            print("Pose landmarks detected:", mp_results.pose_landmarks)
            ...
            feedback.append(f"Feet placement: {feet_placement}")
            feedback.append(f"Knee placement: {knee_placement}")
            ...
            feedback.append(f"Error while detecting squat errors: {e}")
